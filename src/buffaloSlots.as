package src 
{
	...
	public class buffaloSlots {
		...
		public function buffaloSlots(controller:*, id:int, betAmountsArr2:Array = null)
		{
		...
		public function doGetSpinResults(res:Object):void
		{
			...
			oSPIN.buffalo_possible = true;
			oSPIN.buffalo_win = false;
			oSPIN.buffalo_icons = new Array();
		...
		private function doLandWheel(e:TimerEvent=null):void
		{
			...
			var reel_has_buffalo:Boolean = false;
			...
			for(var row:uint=1; row<=4; row++){
				...
				if(myClip.myItem == "F"){
					if(myWheel == 1 || myClip.inOutcome){
						reel_has_buffalo = true;
						oSPIN.buffalo_icons.push(myClip);
					}else{
						myClip.iconF.doStop();
					}
				}
			...
			//----buffalo----------------------
			if(reel_has_buffalo){
				blittools_sounds.playSound("snd_buffalo2", "SFX1");
			}else{
				if(oSPIN.buffalo_win == false){
					for(i = 0; i <oSPIN.buffalo_icons.length; i++){
						oSPIN.buffalo_icons[i].iconF.doStop();
					}
				}
			}
		...
		public function doShowWayResult(res:Object):void
		{
			...
			if(winning_symbol == "C"){
				...
				blittools_sounds.playSound("snd_buffalo", "INTERFACE");
			}else{
